@NotAutomated
@JIRA-Key-MANUSCRIPT-31877
@Notification
Feature: Notification - Notification Template - Action

  Scenario: For each type of notifications the action is shown per each notification in the notifications page
    ...
    Then The following notifications are shown in the "Notifications" page in the following order:
      | Type      | Title                            | Item   |
      | Notify    | 17: C17                          | Notify |
      | Subscribe | W1: Root Article                 | Edit   |
      | Assign    | 16: Intro Case for Administrator | Assign |
