@NotAutomated
@JIRA-Key-MANUSCRIPT-31447
@Planner
Feature: Planner - Kanban - Group By Priority - Filter link

  Scenario: Grouping by "Priority" has filter link in kanban
    ...
    When The user enters the following details for the "Planned Cases" column:
      | Group By |
      | Priority |
    And The user clicks the "1 - Must Fix" link in the "Planned Cases" column
    Then The "Filters" page is shown
    And The following filter is shown:
      | First 200 cases with priority 1 (Must Fix) in milestone Hurricane containing kanban:=0 and (project:"Ninja") |
