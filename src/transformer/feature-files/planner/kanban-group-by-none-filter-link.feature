@NotAutomated
@JIRA-Key-MANUSCRIPT-31445
@Planner
Feature: Planner - Kanban - Group By None - Filter link

  Scenario: Grouping by none has a filter link in kanban
    ...
    When The user enters the following details for the "Planned Cases" column:
      | Group By |
      | None     |
    And The user clicks the "Cases" link in the "Planned Cases" column
    Then The "Filters" page is shown
    And The following filter is shown:
      | First 200 open cases in milestone Hurricane containing kanban:=0 and (project:"Ninja") |
