@NotAutomated
@JIRA-Key-MANUSCRIPT-31064
@Planner
Feature: Planner - Case - Group By Priority - Filter link

  Scenario: Grouping by "Priority" has filter link
    ...
    When The user enters the following details for the "Hurricane" milestone in the "Planner" page:
      | Group By |
      | Priority |
    And The user clicks the "1 - Must Fix" link in the "Hurricane" milestone in the "Planner" page
    Then The "Filters" page is shown
    And The following filter is shown:
      | First 200 cases with priority 1 (Must Fix) in milestone Hurricane containing (project:"Ninja") |
