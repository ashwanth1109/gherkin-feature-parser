@NotAutomated
@JIRA-Key-MANUSCRIPT-31484
@Planner
Feature: Planner - Kanban - Doing - Add New Case

  Scenario: User can create a new untitled case for the "Doing" kanban column
    ...
    And The user clicks the "Add" button for the "Doing" column
    ...
    Then The following cases are shown in the "Doing" column:
      | ID | Title      |
      | 17 | (Untitled) |
    And The user clicks the "(Untitled)" case in the "Doing" column
    And The following case details are shown in the "View Active Case" dialog:
      | ID | Title      | Milestone | Project | Status | Kanban Column |
      | 17 | (Untitled) | Hurricane | Ninja   | Active | Doing         |

  Scenario: User can create a new case with a title for the "Doing" kanban column
    Given The user is logged in as Administrator
    And The following projects exist:
      | Project Name | Primary Contact |
      | Ninja        | Administrator   |
    ...
    Then The following cases are shown in the "Doing" column:
      | ID | Title   |
      | 17 | Another |
    And The user clicks the "Another" case in the "Doing" column
    And The following case details are shown in the "View Active Case" dialog:
      | ID | Title   | Milestone | Project | Status | Kanban Column |
      | 17 | Another | Hurricane | Ninja   | Active | Doing         |
