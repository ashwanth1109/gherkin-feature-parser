@NotAutomated
@JIRA-Key-MANUSCRIPT-32853
@WikiArticle
Feature: Wiki Article - Edit - Insert - Link - Wiki article links are searched by ID
  ...
  Scenario: Administrator user can search wiki article using wiki article ID when adding link in a wiki article
  ...
  Scenario: Administrator user can select wiki article link from title suggestion list when adding link in a wiki article : Search done using wiki article ID
  ...
  Scenario: The wiki article link is successfully added to wiki article description when user selects the link from the suggestion list: Search done using wiki article ID
    Given The user enters the link details in the "Insert Link" editor dialog:
      | Target |
      | W3     |
    And The following suggestions are listed in the "Title Suggestions" input text droplist in the "Insert Link" editor dialog in the "Edit Wiki Article" page:
