@NotAutomated
@JIRA-Key-MANUSCRIPT-31205
@WikiTemplate
Feature: Wiki template - Render Wiki article - Placeholder - rss_url

  Scenario: User can view rendered Wiki article: "$rss_url$" placeholder: URL of RSS feed of current article's changes
    Given The user is logged in as Administrator
    And The following wiki templates exist:
      | Template                | wiki-page-content div HTML |
      | Rendering Wiki template | $rss_url$                  |
    ...
    # NOTE: Most likely, `Destination` value would be different in the new FogBugz.
    And The following wiki article details are shown in the "View Wiki Article" page:
      | Page Title    | Description                                                                         |
      | First Article | default.asp?pg=pgRss&ixWikiPage=2&sSignature=hmacsha1-2-mS3rYByXNgy2cyISk94jc_TuXwk |
